                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('payments.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label for="amount_paid" class="block text-sm font-medium text-gray-700">Monto a
                                    Pagar</label>
                            ...
                            <div>
                                <label for="payment_currency" class="block text-sm font-medium text-gray-700">Moneda del
                                    Pago</label>
                            ...
                            <div>
                                <label for="payment_date" class="block text-sm font-medium text-gray-700">Fecha de
                                    Pago</label>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateExchangeRateVisibility() {
                ...
                if (invoiceCurrency === paymentCurrency) {
                    // Si las monedas son iguales, deshabilitar el campo de tipo de cambio y establecer valor a 1
                    exchangeRateInput.value = '1';
                    exchangeRateInput.disabled = true;
                    exchangeRateInput.classList.add('bg-gray-100');
                } else if ((invoiceCurrency === 'USD' && paymentCurrency === 'NIO') || (invoiceCurrency === 'NIO' && paymentCurrency === 'USD')) {
                    ...
                    if (!exchangeRateInput.value || exchangeRateInput.value === '1') {
                        exchangeRateInput.value = '36.50'; // Ejemplo de tipo de cambio predeterminado
                    }
                }
            ...
            function updateConversionSummary() {
                ...
                if (invoiceCurrency !== paymentCurrency) {
                    if (invoiceCurrency === 'USD' && paymentCurrency === 'NIO') {
                        // Si la factura está en USD y el pago es en NIO
                        convertedAmount = amount / rate;
                        summaryHTML = `
                            <p>Monto ingresado: C$ ${amount.toLocaleString('es-NI', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                        })}</p>
                            <p>Tipo de cambio: C$ ${rate.toLocaleString('es-NI', {
                            minimumFractionDigits: 4,
                            maximumFractionDigits: 4
                        })} = $1 USD</p>
                            <p>Equivalente en USD: $ ${convertedAmount.toLocaleString('es-NI', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                        })}</p>
                        `;
                    } else if (invoiceCurrency === 'NIO' && paymentCurrency === 'USD') {
                        // Si la factura está en NIO y el pago es en USD
                        convertedAmount = amount * rate;
                        summaryHTML = `
                            <p>Monto ingresado: $ ${amount.toLocaleString('es-NI', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                        })}</p>
                            <p>Tipo de cambio: C$ ${rate.toLocaleString('es-NI', {
                            ...
                            <p>Equivalente en NIO: C$ ${convertedAmount.toLocaleString('es-NI', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                        })}</p>
                        `;
                    ...
                    // Si las monedas son iguales (sin conversión)
                    convertedAmount = amount;
                    const currencySymbol = paymentCurrency === 'USD' ? '$' : 'C$';
                    summaryHTML = `
                        <p>Monto a pagar: ${currencySymbol} ${amount.toLocaleString('es-NI', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    })}</p>
                    `;
                ...
                // Calcular el porcentaje del saldo pendiente
                const percentage = (convertedAmount / invoiceAmount) * 100;
                summaryHTML += `
                    <p class="mt-2 font-semibold ${percentage >= 100 ? 'text-green-600' : ''}">
                        Este pago representa el ${percentage.toFixed(2)}% del saldo pendiente (${invoiceCurrency === 'USD' ? '$' : 'C$'}${invoiceAmount.toLocaleString('es-NI', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                })} ${invoiceCurrency})
                    </p>
                ...
                // **Nuevo cálculo:** Mostrar siempre el equivalente en dólares si la factura está en córdobas
                if (invoiceCurrency === 'NIO') {
                    const equivalentInDollars = invoiceAmount / rate;
                    summaryHTML += `
                        <p class="font-semibold text-blue-600">
                            El saldo pendiente en USD es: $${equivalentInDollars.toLocaleString('es-NI', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    })}
                        </p>
                ...
                // Actualizar el resumen de conversión
